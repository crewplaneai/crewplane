"""Validate persisted preflight plan shape and version markers."""
...
def validate_current_runtime_snapshot_shape(snapshot: JsonObject) -> None:
...
def validate_current_fingerprint_metadata_shape(metadata: JsonObject) -> None:
    _reject_legacy_plan_fields(
        metadata,
...
def validate_current_value_fingerprint_shape(
...
def _validate_persisted_input_node_contract(node: object) -> None:
    ...
    if len(input_sources) != 1:
        raise ValueError(
            ...
            f"'{plan_contract_records.node_id(node)}' must define exactly one "
...
def _validate_persisted_provider_node_contract(node: object) -> None:
    if not str(getattr(node, "render_plan_id", "") or "").strip():
        raise ValueError(
            ...
            f"'{plan_contract_records.node_id(node)}' must define render_plan_id."
        )
    if not getattr(node, "provider_records", ()):
        raise ValueError(
            "Persisted provider preflight node "
            f"'{plan_contract_records.node_id(node)}' must define at least one "
            "provider record."
        )
