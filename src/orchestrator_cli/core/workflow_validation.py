...
from .prompt_segments import PromptSegmentRole
from .token_budget import resolve_token_budget
from .workflow_graph import ancestor_map, topological_waves
...
def _validate_single_sequential_provider_role(node: WorkflowNode) -> None:
    ...
    if role == "executor":
        return
    raise ValueError(
        f"Sequential node '{node.id}' has a single provider but role is '{role}'. "
        "Role must be 'executor' for single-provider nodes."
    )
...
def validate_audit_rounds_settings(workflow: WorkflowPlan, config: Config) -> None:
    """Validate sequential node audit rounds against config limits."""
...
def _validate_unique_node_ids(workflow: WorkflowPlan) -> None:
    ...
    raise ValueError(f"Duplicate node IDs found: {duplicate_labels}")
...
def collect_provider_validation_errors(
    ...
    """Collect provider resolution errors for a workflow plan."""

    missing_provider_locations: dict[str, list[str]] = {}

    for node in workflow.nodes:
        for provider in node.providers:
            _collect_provider_reference_errors(
                workflow_name=workflow.name,
                node_id=node.id,
                provider_name=provider.provider,
                config=config,
                missing_provider_locations=missing_provider_locations,
            )

    return _format_provider_validation_errors(
        missing_provider_locations=missing_provider_locations,
    )
...
def collect_token_budget_validation_errors(
    ...
    """Collect token budget errors for a workflow plan."""
...
def _collect_provider_reference_errors(
    workflow_name: str,
    node_id: str,
    provider_name: str,
    config: Config,
    ...
    location = f"workflow '{workflow_name}' -> node '{node_id}'"
    agent_config = config.agents.get(provider_name)
    if agent_config is None:
        missing_provider_locations.setdefault(provider_name, []).append(location)


def _format_provider_validation_errors(
    missing_provider_locations: dict[str, list[str]],
) -> list[str]:
    return _format_unknown_provider_errors(missing_provider_locations)
...
def validate_provider_references(
    ...
    """Validate that workflow provider references exist in config."""
...
def validate_token_budget_settings(workflow: WorkflowPlan, config: Config) -> None:
    """Validate workflow token budget settings."""
