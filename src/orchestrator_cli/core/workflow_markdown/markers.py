...
from ..prompt_segments import PromptSegmentPayload, PromptSegmentRole
from .models import (
...
ORCHESTRATOR_OPEN_MARKER_PATTERN = re.compile(
    r"^<!--\s*orchestrator:([a-z0-9._-]+)\s*-->$"
)
ORCHESTRATOR_CLOSE_MARKER_PATTERN = re.compile(
    r"^<!--\s*/orchestrator:([a-z0-9._-]+)\s*-->$"
)
ORCHESTRATOR_MARKER_NAMESPACE_PATTERN = re.compile(
    r"^<!--\s*/?\s*orchestrator\s*:",
    re.IGNORECASE,
)
...
def validate_marker_role(raw_role: str, source: Path, node_id: str) -> PromptMarkerRole:
    if raw_role not in ALLOWED_PROMPT_MARKER_ROLE_SET:
        ...
        raise ValueError(
            f"{source} node '{node_id}' declares unknown orchestrator role marker "
...
def collect_marker_events(
    ...
    for token in COMMONMARK_PARSER.parse(section_text):
        ...
        open_match = ORCHESTRATOR_OPEN_MARKER_PATTERN.fullmatch(marker_text)
        ...
        close_match = ORCHESTRATOR_CLOSE_MARKER_PATTERN.fullmatch(marker_text)
        ...
        if ORCHESTRATOR_MARKER_NAMESPACE_PATTERN.match(marker_text) is not None:
            raise ValueError(
                f"{source} node '{node_id}' contains malformed orchestrator marker "
                f"'{marker_text}'."
            )
    return events
    ...
    for event in events:
        ...
        if event.marker_kind == "open":
            if active_role is not None:
                raise ValueError(
                    f"{source} node '{node_id}' contains nested orchestrator role markers."
                )
        ...
        if event.marker_kind == "close":
            if active_role is None:
                raise ValueError(
                    f"{source} node '{node_id}' contains an orchestrator role close marker "
