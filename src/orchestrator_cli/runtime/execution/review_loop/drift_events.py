...
def path_relative_to_orchestrator_root(
    output: ArtifactStorePort,
    path: Path,
) -> str:
    orchestrator_root = output.stages_dir.parent.parent
    try:
        return str(path.relative_to(orchestrator_root))
    except ValueError:
        return str(path)
...
def format_drift_paths(
    ...
    display_paths = [
        path_relative_to_orchestrator_root(output, path) for path in paths[:5]
    ]
    if len(paths) > 5:
