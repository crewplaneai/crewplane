...
from .worktree_refs import checked_ref, safe_file_component, safe_ref_component
from .worktree_types import WorktreeCaptureRequest, WorktreeSourceRef
...
def verify_source_commit_available(
    ...
    with TemporaryDirectory(prefix="orchestrator-cli-lineage-verify-") as temp_dir:
...
def _import_ref_for_source_commit(source_commit: str) -> str:
    return f"refs/orchestrator-cli/imported/{safe_ref_component(source_commit[:24])}"
