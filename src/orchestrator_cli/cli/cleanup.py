...
from orchestrator_cli.core.config import Settings, load_config
...
def cleanup_workspaces(
    ...
    console = Console()
    try:
        resolved_config_file = resolve_orchestrator_file(
            config_file,
            "config.yml",
...
def project_root_for_config(config_file: Path) -> Path:
    if config_file.parent.name == ".orchestrator":
...
def workspace_status_lookup(project_root: Path) -> WorkspaceStatusLookup:
    stage_root = project_root / ".orchestrator" / "execution-stages"
