...
def write_fake_gh(path: Path) -> None:
    shutil.copyfile(ROOT / "tests" / "fixtures" / "packaging" / "fake_gh.py", path)
    path.chmod(0o755)


def write_fake_uv(path: Path) -> None:
    shutil.copyfile(ROOT / "tests" / "fixtures" / "packaging" / "fake_uv.py", path)
    path.chmod(0o755)
...
def run_release(
    ...
    result = run_process(
        [str(GITHUB_RELEASE_SCRIPT), str(dist)], cwd=ROOT, check=False, env=env
    )
