    ...
    async def test_single_provider_sequential_respects_depth_and_sanitizes_filename(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            node = WorkflowNode(
                ...
                depth=3,
                providers=[ProviderSpec(provider="solo/provider", role="executor")],
            )
    ...
    async def test_sequential_review_loop_uses_role_scoped_authored_prompt_content(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            node = WorkflowNode(
                ...
                mode="sequential",
                prompt_segments=[
                    PromptSegment(role="shared", content="Shared context.\n"),
                    PromptSegment(role="executor", content="Executor delta.\n"),
                    PromptSegment(role="reviewer", content="Reviewer delta.\n"),
                ],
                depth=1,
                providers=[
                    ProviderSpec(provider="exec", role="executor"),
    ...
    async def test_review_loop_fails_when_resolved_reviewer_prompt_is_empty(
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            node = WorkflowNode(
                ...
                mode="sequential",
                prompt_segments=[
                    PromptSegment(role="executor", content="Executor instructions."),
                    PromptSegment(
                        role="reviewer", content="{{env:EMPTY_REVIEWER_PROMPT}}"
                    ),
                ],
                depth=1,
                providers=[
