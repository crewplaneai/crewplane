...
Crewplane is a control plane for resumable coding-agent workflows. The core
model is blackboard orchestration: providers do not coordinate through shared
in-memory state. Each provider invocation receives a rendered prompt and writes
...
- Workflow Markdown and `.crewplane/config.yml` compile into a preflight plan.
- The workflow DAG decides which nodes are ready to run.
- The configured invoker runs each node with deterministic `mock` output or real
  `cli` provider commands.
- Outputs, logs, manifests, and results are written into the run record.
- Downstream nodes read those files through explicit artifact references such as
  `{{node.output}}`.
...
An `agent` is the named config entry that holds the command and provider kind.
A workflow `provider` name selects one of those agents. In beginner examples,
keep those names identical, such as an agent named `mock` and
`providers: ["mock"]`.
...
See [Running Workflows](../guides/running-workflows.md) for how preflight shapes
execution decisions. See the
...
Continue to [Installation](installation.md) to install Crewplane and prepare for
the provider-free quickstart.
