...
## Context
When the orchestrator executes a workflow, it saves an execution manifest keyed by the compiled `workflow_signature`. If a future run produces the same successful signature, the system suppresses execution to save time and API costs (idempotency/caching).
Previously, if the stored `/manifests/{workflow_signature}.json` file was corrupted (e.g., due to a process crash during write, manual editing, or disk issues), the system might crash or incorrectly assume the run should be skipped, leaving the workflow in an unrecoverable "skipped but broken" state. This addresses: "Corrupt Manifest Skip Behavior" from the architecture review.
...
## Updates
- **2026-04-10**: Implemented robust exception handling (`json.JSONDecodeError`, `UnicodeDecodeError`) within the `manifest_exists` check in `artifacts/manager.py`.
