...
## Context
When Crewplane executes a workflow, it saves current run state in `/manifests/run.json` with the compiled `workflow_signature`. If a future run produces the same successful signature, the system suppresses execution to save time and API costs (idempotency/caching).
...
3. **Resilience:** Crewplane execution engine should not crash completely just because an old artifact file is unreadable.

## Consequences
### Positive
- Workflow execution gracefully recovers from interrupted writes or corrupted state directories.
- Users no longer need to manually clear the `.crewplane/execution-stages/` directory to recover from a broken JSON manifest.
